"""
Propensity Score Matching: Recovering True Causal Effects (Modified to save plots)
...
from save_plots import save_figure
...
class PropensityScoreMatching:
    ...
    def visualize_propensity_scores(self, propensity_scores, treatment):
        ...
        plt.tight_layout()
        save_figure(plt.gcf(), '04_propensity_scores.png')
        plt.close()
        return auc
    ...
    def visualize_balance(self, balance_stats):
        ...
        plt.tight_layout()
        save_figure(plt.gcf(), '05_covariate_balance.png')
        plt.close()
...
def run_psm_analysis():
    ...
    plt.subplot(2, 2, 1)
    methods = ['Naive\\n(BIASED)', 'PSM\\n(CAUSAL)', 'True\\n(ACTUAL)']
    effects = [naive_effect*100, effect_result['ate']*100, true_effect*100]
    ...
    plt.subplot(2, 2, 2)
    naive_bias = abs(naive_effect - true_effect) * 100
    psm_bias = abs(bias) * 100
    bias_labels = ['Naive', 'PSM']
    bias_values = [naive_bias, psm_bias]
    colors = ['red', 'green']

    bars = plt.bar(bias_labels, bias_values, color=colors, alpha=0.7, edgecolor='black')
    ...
    plt.subplot(2, 2, 3)
    ci_lower = effect_result['ate'] - 1.96 * effect_result['std_error']
    ci_upper = effect_result['ate'] + 1.96 * effect_result['std_error']

    plt.errorbar([0], [effect_result['ate']*100],
                 xerr=[[(effect_result['ate'] - ci_lower)*100], [(ci_upper - effect_result['ate'])*100]],
    ...
    # Plot 4: Balance improvement
    plt.subplot(2, 2, 4)
    before_abs = [abs(balance_stats['before_matching'][f]['std_diff']) for f in features]
    after_abs = [abs(balance_stats['after_matching'][f]['std_diff']) for f in features]

    x = np.arange(len(features))
    width = 0.35

    plt.bar(x - width/2, before_abs, width, label='Before',
            color='lightcoral', edgecolor='black', alpha=0.8)
    plt.bar(x + width/2, after_abs, width, label='After',
            color='lightgreen', edgecolor='black', alpha=0.8)
    ...
    plt.xticks(x, [f[:8] for f in features], rotation=45)
    plt.legend()

    plt.tight_layout()
    save_figure(plt.gcf(), '06_psm_results_summary.png')
    plt.close()
